...
const LINEAGE_BLUEPRINTS: LineageBlueprint[] = [
  {
    key: 'ingest',
    title: 'Step 0 & 1: Ingest & De-identify',
    subtitle: 'Column Mapping & PII Scrubbing',
    description: 'BiolinkStep0ColumnMappingProcessor applies CDISC SDTM/LOINC terminology mapping, followed by BiolinkStep1RemovePIIProcessor for direct PII removal.',
    inputs: ['db/BHS_Full.csv', 'db/EHVol_Full.csv', 'bhs_col_map.py', 'ehvol_col_map.py'],
    outputs: ['Standardized schema JSON', 'Scrubbed PII manifest', 'Completeness score'],
    owner: 'NiFi Step 0 & Step 1 Processors',
    icon: Database,
  },
  {
    key: 'profile',
    title: 'Step 2 & 3: Sparsity & Normalization',
    subtitle: 'Structure & Profile Cleaning',
    description: 'BiolinkStep2ReduceSparseColumnsProcessor filters empty attributes, followed by BiolinkStep3ProfileNormalizationProcessor for demographic and clinical normalization.',
    inputs: ['Standardized record stream', 'Missingness thresholds'],
    outputs: ['Reduced attribute set', 'Normalized clinical flags'],
    owner: 'NiFi Step 2 & Step 3 Processors',
    icon: GitBranch,
  },
  {
    key: 'unify',
    title: 'Step 4 & 5: Ranges & Units',
    subtitle: 'Validation & UCUM Extraction',
    description: 'BiolinkStep4ApplyRangeRulesProcessor validates physiological bounds (BP, EF %, HR), followed by BiolinkStep5ExtractUnitsProcessor for measurement unit extraction.',
    inputs: ['Normalized cohort streams', 'Physiological range rules'],
    outputs: ['Outlier audit flags', 'UCUM standardized units (mg/dL, mmHg)'],
    owner: 'NiFi Step 4 & Step 5 Processors',
    icon: Layers3,
  },
  {
    key: 'quality',
    title: 'Step 6 & 7: Fuzzy Matching & Unification',
    subtitle: 'Entity Linkage & PostgreSQL Load',
    description: 'BiolinkStep6FuzzyMatchProcessor performs multi-cohort subject resolution, followed by BiolinkStep7UnifyDatasetsProcessor for wide table assembly and DB load.',
    inputs: ['Harmonized cohort records', 'Linkage anchors'],
    outputs: ['unified_registry table', 'cohort_comparability', 'registry_etl_runs'],
    owner: 'NiFi Step 6 & Step 7 Processors',
    icon: FlaskConical,
  },
  {
    key: 'publish',
    title: 'Publish & Analytics',
    subtitle: 'AI Agent & Consumer Access',
    description: 'Exposes harmonized PostgreSQL datasets to FastAPI backend, LangGraph AI Agents, and embedded Apache Superset analytics dashboards.',
    inputs: ['unified_registry (PostgreSQL)', 'Vector embeddings (pgvector)'],
    outputs: ['FastAPI REST Endpoints', 'Research Portal Analytics', 'Superset Dashboards'],
    owner: 'FastAPI + LangGraph AI Agent',
    icon: Send,
  },
];

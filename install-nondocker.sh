    ...
    if [[ "$OSTYPE" == darwin* ]]; then
        ...
        if ! pg_isready -h localhost -p 5432 >/dev/null 2>&1; then
            ...
            if [ -f "$PG_CONFIG_FILE" ]; then
                PG_OPTS="-c config_file=$PG_CONFIG_FILE -c hba_file=$PG_HBA_FILE -c ident_file=$PG_IDENT_FILE"
            fi
            if [ -n "$PG_OPTS" ]; then
                (cd /tmp && sudo -u postgres /usr/lib/postgresql/14/bin/pg_ctl -w -D /var/lib/postgresql/14/main -l /tmp/postgres.log -o "$PG_OPTS" start)
            else

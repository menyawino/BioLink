...
class SQLAgentService:
    ...
    def _build_graph(self):
        ...
        graph.add_conditional_edges(
            "execute_sql", self.handle_sql_error, {"retry": "generate_sql", "fail": END}
        )
    ...
    def query_router(self, state: AgentState) -> AgentState:
        ...
        if not needs_sql:
            state["final_response"] = (
                "I can help you with patient data queries. Try asking about patient counts, averages, or specific data analysis."
            )
            return state
    ...
    def _heuristic_sql(self, query_lower: str) -> Optional[str]:
        ...
        if "how many" in query_lower or (
            "count" in query_lower and "patient" in query_lower
        ):
        ...
        if "age distribution" in query_lower or (
            "age" in query_lower and "distribution" in query_lower
        ):
        ...
        if (
            ...
            and "ef" in query_lower
            and ("male" in query_lower or "female" in query_lower)
        ):
            ...
            if result:
                ...
                if (
                    ...
                    try:
                        ...
                        if isinstance(parsed_result, list) and len(parsed_result) > 0:
                            if isinstance(parsed_result[0], tuple):
                                ...
                                if len(parsed_result[0]) == 1:
                                    # Single column
                                    df = pd.DataFrame(
                                        [row[0] for row in parsed_result],
                                        columns=["result"],
                                    )
                                else:
                                    ...
                                    df = pd.DataFrame(
                                        ...
                                        columns=[
                                            f"col_{i}"
                                            for i in range(len(parsed_result[0]))
                                        ],
                                    )
                        ...
                        logger.error(
                            f"Failed to parse result with ast: {e}, result_str: {result_str[:500]}"
                        )
    ...
    def viz_decider(self, state: AgentState) -> AgentState:
        ...
        state["viz_config"] = {"plot_type": "histogram", "title": "Data Visualization"}
        return state
            ...
            if "age" in query_lower and "gender" in query_lower:
                ...
                if "age" in df.columns and "gender" in df.columns:
                    ...
                    if not df_clean.empty:
                        ...
                        plt.hist(
                            [
                                df_clean[df_clean["gender"] == "Male"]["age"],
                    ...
                    plt.text(
                        0.5,
                        0.5,
                        "Age or gender column not found",
                        ha="center",
                        va="center",
                    )
                ...
                if "age" in df.columns:
                    ...
                    if not df_clean.empty:
                        ...
                        plt.text(
                            0.5, 0.5, "No valid age data", ha="center", va="center"
                        )
                else:
                ...
                if len(numeric_cols) >= 2:
                    ...
                    plt.text(
                        0.5,
                        0.5,
                        "Not enough numeric data for plotting",
                        ha="center",
                        va="center",
                    )
    ...
    def respond(self, state: AgentState) -> AgentState:
        ...
        # `user_query` not required for final response generation
        ...
        if error:
            response = (
                f"I encountered an error: {error}. Please try rephrasing your query."
            )
        else:

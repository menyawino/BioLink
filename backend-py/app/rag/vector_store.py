    ...
    with get_pg_conn() as conn:
        with conn.cursor() as cur:
            execute_values(
                ...
                """,
                [
                    (pid, chunk_id, content, json.dumps(meta), embedding)
                    for pid, chunk_id, content, meta, embedding in rows
                ],
            )
        conn.commit()
...
def similarity_search(
    query_embedding: list[float], k: int = 5, patient_ids: list[str] | None = None
):
    with get_pg_conn() as conn:
        with conn.cursor() as cur:
